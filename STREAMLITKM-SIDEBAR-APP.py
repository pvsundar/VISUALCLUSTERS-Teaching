...
feature_names_input = st.sidebar.text_input("Enter names for the two features separated by a comma", "Ease of Use, Sophisticated Technology")
n_samples = st.sidebar.slider("Number of data points", 50, 500, 100)
cluster_std = st.sidebar.slider("Standard deviation for the clusters", 0.5, 3.0, 2.0)
max_iter = st.sidebar.slider("Maximum number of iterations", 2, 20, 6)
initial_centroids_input = st.sidebar.text_input("Initial centroids (x,y)", "9,8; -2,-2")
...
2. Run the app using 'streamlit run STREAMLITKM-SIDEBAR-APP.py' in your command line or terminal.
""")
